...
- For exploratory POCs where trade timing, participation, state behavior, or exit quality is central to the question, include representative trade tapes in the summary deck or report whenever practical. Prefer a small, readable set of panels that shows the key behavior over a dense all-symbol contact sheet alone.
- When a summary slide deck covers multiple runs, experiment phases, or conceptual pivots, include clear transition slides between those sections so the operator can see the shift at a glance during later review.
