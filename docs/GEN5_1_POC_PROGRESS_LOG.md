...
3. Keep behavioral-pool PCA and k-means variants in contention.
4. Treat `pca_kmeans_auto` as promising, but not accepted, until it survives confirmation runs.
5. Continue using portfolio accounting only as a downstream inspection layer.

## Next Candidate Tests

- Confirm active-plus-risk behavioral-pool `pca_kmeans_auto` on a shifted end date or adjacent data window.
- Compare auto-k against fixed k9 under a modestly expanded strategy grid.
