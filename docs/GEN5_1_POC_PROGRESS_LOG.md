...
| 25. SIP Historical Coverage Repair | `ACCEPTED_DIRECTION` | Was the apparent history limit caused by Gen5 using IEX by default while Gen4 auto-selected SIP? | [refresh audit](../runs/data_refresh/alpaca_daily_audit_20260623.csv), [coverage CSV](../runs/data_refresh/alpaca_daily_symbol_coverage_20260623.csv), [refreshed deck](../outputs/gen5_temporal_context_replication_summary_sip_refresh.pptx) | Confirmed with live Alpaca probes that SIP returns 2016-era bars where IEX did not. Gen5 now defaults the research feed to SIP while preserving `ALPACA_DATA_FEED` overrides. Refreshed cache shows `AMD,NVDA,TSLA,AAPL,MSTR,SPY,QQQ,IWM,SMH,TLT,GLD` from `2016-01-04`; `VXX` starts `2018-01-18`. A broad older-window rerun was started and stopped after proving data viability because compute became the limiting factor; run the expanded 2021/2022 replication as an overnight batch after deciding how to handle `VXX` for pre-2018 windows. |
