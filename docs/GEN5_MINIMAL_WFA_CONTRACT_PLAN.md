...
Implementation note: the first AMD EMA OOS signal/position application evidence slice now exists as `R/wfa_amd_ema_oos_signal_position_application.R`. It consumes the accepted frozen-parameter application boundary and canonical Alpaca adjusted daily AMD bars with the same explicit `as_of_timestamp`, applies the already-frozen fast/slow EMA periods to OOS sessions, preserves `no_trade_cash` as first-class per-session rows, and records signal plus next-open position-state evidence only. It fails on missing frozen OOS rows, duplicate bars, future or mismatched timestamps, and missing next-session dates. It does not compute returns, PnL, Sharpe, drawdown, allocation, leverage, live advice, execution, dashboards, broader strategy families, or performance claims.

Implementation note: the first AMD EMA OOS session measurement value slice now exists inside `R/wfa_amd_ema_oos_measurement_contract.R`. It consumes the accepted measurement contract, frozen AMD EMA OOS signal/position evidence, and canonical Alpaca adjusted daily AMD bars with the same explicit `as_of_timestamp`; emits row-per-session measurement values only for the already-authorized session fields; preserves `no_trade_cash` as first-class zero-return rows; and gates candidate strategy session return by the frozen position state. It fails on missing signal/position evidence, missing canonical AMD bars, mismatched timestamps, unknown application lineage, duplicate bars, future data, or row-count mismatches. It does not compute trade lifecycle rows, trade PnL, trade returns, Sharpe, drawdown, fold/global summaries, allocation, leverage, live advice, execution, dashboards, broader strategy families, or performance claims.

Implementation note: the first AMD EMA OOS trade lifecycle scaffold now exists inside `R/wfa_amd_ema_oos_measurement_contract.R`. It consumes the accepted measurement contract and frozen AMD EMA OOS signal/position evidence, derives entry and exit lifecycle rows from candidate long/cash transitions only, records deterministic trade IDs, entry/exit signal and execution dates, trade status, position state, and holding-period session counts where the lifecycle is closed, and leaves share quantity, trade PnL, and trade open-to-open return uncomputed. It does not compute performance metrics, fold/global summaries, allocation, leverage, live advice, execution, dashboards, broader strategy families, or performance claims.
