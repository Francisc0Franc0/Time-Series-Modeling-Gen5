...
Implementation note: the first AMD EMA long/cash evaluation contract surface now exists as `R/wfa_amd_ema_evaluation_contract.R`. It consumes the accepted AMD EMA gate plus fold geometry, TRAIN/OOS split audit availability, frozen fold evidence, and no-trade baseline contract rows. It emits schema-only manifest, review rows, and a readiness review with `no_trade_cash` first for every fold and `amd_ema_long_cash` second, deterministic ignored-run artifact paths, review-required reasons, and leakage attestations. It does not compute EMA signals, returns, cash yields, trade accounting, benchmark math, performance metrics, allocation, leverage, dashboards, execution, live advice, broader strategy families, or performance claims.
