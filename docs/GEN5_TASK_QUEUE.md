...
- `scripts/inspect/run_pca_comparison_report.ps1` runs or consumes the four `PanelMode x StateMap` combinations and writes a compact comparison packet under ignored `runs/research_workbench/regime_wfa_comparisons/`, including OOS metrics, state coverage, selected-family counts, child artifact paths, and 2x2 equity/OOS/PCA visual contact sheets.
