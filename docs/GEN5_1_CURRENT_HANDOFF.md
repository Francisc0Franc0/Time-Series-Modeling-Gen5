...
- The newest Gen4-forensic POC is Gen4-equivalence Stage 1. Wrapper: `scripts/inspect/run_gen4_equivalence_screen.R`; packet: `runs/research_workbench/gen4_equivalence/gen4_equivalence_20260706stage1c/`; deck: `presentations/gen5_gen4_equivalence_stage1.pptx`. It asks whether remembered Gen4 benchmark-beating behavior came from a real design difference or from unlike setup/reporting. The wrapper uses the Gen4 `RESEARCH_ASSETS` analogue as behavioral-pool context, 4x4 quantile states, expanding TRAIN from 2016Q4, and compares direct-spec versus pooled-family selection. Full 178-row all-symbol/all-quarter compute was too slow for a first pass, so Stage 1 used AMD/NVDA/TSLA across `2020Q4`, `2022Q2`, and `2024Q4` with an 80-row grid reconstructed from supported Gen4-picked strategy labels. Readout: direct-spec returned `30.7%` versus equal-weight hold `-15.2%` (`+45.9 pp` alpha), pooled-family returned `5.0%` (`+20.2 pp`). Follow-up source/artifact audit found the current Gen5.1 preset was missing several active Gen4 artifact parameter values, especially Bollinger lookback 10, return-z entry 1.5, breakout lookback 10, and nonzero breakout buffers; this is now corrected for implemented non-SMA families. This is promising but not resolved equivalence evidence. SMA families remain absent from Gen5.1, exact Gen4 volatility-breakout semantics remain a possible gap, and PLTR/SOFI partial-history WARNs are expected listing-history limitations in the broad context universe. Recommended next step is to expand the corrected picked-spec grid to the full Gen4 high-beta cluster over a larger annual-quarter ladder before implementing SMA or spending full 16-symbol compute.
