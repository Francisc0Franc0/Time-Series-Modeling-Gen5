...
- The newest Gen5.2 calibration packet is `runs/research_workbench/gen4_equivalence/gen4_equivalence_gen52calfull162024q420260707/`. It reruns the full 16-symbol Gen4-equivalence slice for `2024Q4` with Gen5.2 direct and pooled selection rebuilt from TRAIN performance tables, not relabeled from cached old selected states. The wrapper now supports `GEN5_GEN4_EQ_GEN4_SUMMARY_SCOPE=selected_quarters`, which normalizes the Gen4 artifact over the same `2024-10-01` to `2024-12-31` comparison window as Gen5.2. The Gen4 and Gen5.2 live/reporting baskets are exactly identical: `AMD,EFA,GLD,IWM,KO,NVDA,PEP,PLTR,QQQ,SOFI,SPY,TLT,TSLA,WMT,XLE,XLF`. Cluster 1 and cluster 3 are reporting lenses from a separate Gen4 asset-level PCA/clustering map; they are applied after symbol-level replay to aggregate/visualize equity and do not feed state fitting, strategy selection, or simulated trades. Readout: cluster 1 is close across lanes, but cluster 3 still shows a large upside-participation gap versus Gen4. The follow-up audit lives under `runs/research_workbench/gen4_equivalence/gen4_equivalence_gen52calfull162024q420260707/trade_tape_audit/`, with deck `presentations/gen5_2_mechanics_and_gen4_calibration.pptx`. It shows that on shared cluster-3 symbols `AMD,NVDA,PLTR,SOFI,TSLA`, the remaining gap is symbol-specific rather than simply global underexposure: Gen4 was long SOFI for `89%` of the quarter while both Gen5.2 lanes were `0%` long SOFI; Gen5.2 redirected exposure toward PLTR and TSLA. Next recommended forensic slice: inspect SOFI and PLTR selected state/spec rows, TRAIN eligibility, no-trade competition, and Gen4 picked family/params. This remains research/inspection only and does not yet write a true live-capital portfolio-accounting packet.
