...
- A Gen4-vs-Gen5.1 selection-policy fork is now documented in `docs/GEN5_1_SELECTION_POLICY_HYPOTHESIS.md`. Gen4 Phase50 appears to use `pooled_family_asset_variant`: choose a state-level family from pooled evidence, then choose asset-specific parameters. Current Gen5.1 uses `asset_state_direct_spec`: choose the best full asset/state strategy spec directly. The first paired screen is under `runs/research_workbench/selection_policy_screens/selection_policy_screen_A5_Q2Q3_20260702/`, with visual summaries under `runs/research_workbench/selection_policy_screens/selection_policy_screen_A5_Q2Q3_20260702/visual_summary/`: direct and pooled-family maps matched on `321 / 375` asset-state rows (`85.6%`), but replay behavior still diverged in visible ways. A broader resumable wrapper now exists at `scripts/inspect/run_selection_policy_robustness_screen.R`, with lanes `A_live` and `B_hist`; the first new `A_live` authority build was compute-bound and stopped before completed evidence. Do not change the live bridge default until the broader wrapper completes or the operator explicitly decides to stop at the first paired screen.
...
```text
Please continue on branch codex/Gen5.1-context-universe-factorial-plan. Read AGENTS.md, docs/GEN5_1_CURRENT_HANDOFF.md, and docs/GEN5_1_SELECTION_POLICY_HYPOTHESIS.md first. Run the selection-policy robustness wrapper as a resumable job starting with GEN5_SELECTION_POLICY_ONLY=A_live. Do not change live bridge behavior. If the run completes, inspect the report and visual summaries; if it remains too slow, add per-symbol authority checkpointing before starting B_hist.
```
