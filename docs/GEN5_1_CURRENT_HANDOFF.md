...
- Live bridge freeze guard: the dual bridge direct-spec lane is operationally frozen to consume `bridge_selected_states.csv` and must not recompute direct selection from `bridge_train_state_performance.csv` during daily advice. Gen5.X research may alter research selection helpers, but live bridge selection/replay/continuity semantics require an explicit live-bridge change. Daily packets now include runtime provenance (`branch`, git SHA, dirty status, and `live_bridge_code_version`). See the incident guardrail in `docs/GEN5_1_LIVE_ADVICE_BRIDGE.md`.
