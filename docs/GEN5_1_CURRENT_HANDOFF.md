...
- The newest troubleshooting POC is the EMA trend participation probe, using `HB_broad_risk_no_vxx` as the audit testbed. It deliberately narrows candidate families to `ema_trend,no_trade` before spending full-family compute. Baseline default EMA-only packet: `runs/research_workbench/selpol_context/selpol_context_e0/`; compact participation probe packet: `runs/research_workbench/selpol_context/selpol_context_e2/`; comparison report: `runs/research_workbench/selpol_context/selpol_context_e2/ema_trend_participation_comparison/HB_broad_risk_no_vxx/ema_trend_participation_comparison_report.md`. Readout: compact variants increased EMA-state selection and exposure and modestly improved mean replay proxy (`+0.9 pp` direct, `+1.2 pp` pooled), but window-level deltas were mixed. Keep compact EMA participation variants as a promising mechanism probe; do not promote the broad all-combinations EMA grid or change live behavior from this alone.
