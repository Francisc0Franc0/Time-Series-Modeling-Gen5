...
- Gen5.2 is now opened as the active mechanics fork. See `docs/GEN5_2_DESIGN_BRIEF.md`. The public `pooled_family_asset_variant` policy now uses a Gen4 Phase40-style recipe: TRAIN-only candidate scoring, active rows eligible only with at least five TRAIN trades, no-trade rows allowed as abstention competitors, family picked by mean score/return/variant count, and asset-specific variants picked inside the selected family. Gen4's `no_trade_exit_immediate` concept is represented as an explicit `force_exit_next_open` state exit override in PCA-routed replay and live-advice replay. Keep `asset_state_direct_spec` as a research comparator, but do not assume it is the Gen4-faithful default.
