# Run one frozen LIT-MR-03.2 triplet lane.
...
source(file.path(repo_root, "literature_studies", "R", "gen5_lit_mr_03_1_triplet_poc.R"))
source(file.path(repo_root, "literature_studies", "R", "gen5_lit_mr_03_2_relaxed_triplet_poc.R"))
...
plot_gate_yield <- function(batch, path) {
  ...
  pass_counts <- vapply(paste0("R", 1:8), function(id) {
    sum(detail$gate_id == id & detail$status == "PASS")
  }, numeric(1))
  png(path, width = 1800, height = 1000, res = 150)
  old <- par(mar = c(7, 6, 4, 2))
  barplot(
    pass_counts,
    names.arg = paste0("R", 1:8),
    col = ifelse(pass_counts == nrow(batch$summary), colors$green, colors$blue),
    ylim = c(0, nrow(batch$summary)),
    ylab = "Triplets passing gate",
    main = paste0(
      "LIT-MR-03.2 ", gsub("_", " ", batch$lane),
      ": mandatory-gate yield"
    )
  )
  abline(h = nrow(batch$summary), col = colors$slate, lty = 2)
  mtext(
    "R1 integrity | R2 I(1) | R3 rank | R4 cosine | R5 half-life | R6 support | R7 q10 return | R8 q90 convergence",
    side = 1, line = 4.7, cex = 0.78, col = colors$slate
  )
  par(old)
  dev.off()
}
...
plot_evidence_plane <- function(summary, path) {
  ...
  point_shapes <- ifelse(summary$rank_one, 19, 1)
  png(path, width = 1800, height = 1100, res = 150)
  old <- par(mar = c(6, 6, 4, 2))
  plot(
    x, y,
    pch = point_shapes, cex = 1.2, col = point_colors,
    xlab = "TRAIN trade bootstrap q10 (bp/trade)",
    ylab = "TRAIN forward-convergence bootstrap q90",
    main = "Favorable uncertainty is necessary; structural gates still apply"
  )
  ...
  if (any(summary$relaxed_full_pass)) {
    ...
    text(
      ...
      passed$forward_q90,
      labels = passed$triplet_id,
      pos = 3, cex = 0.75, col = colors$green
    )
  }
  legend(
    "topright", c("Rank one", "Not rank one"),
    pch = c(19, 1), col = colors$navy, bty = "n"
  )
  par(old)
  dev.off()
}
...
plot_development <- function(summary, path) {
  ...
  colnames(values) <- summary$triplet_id
  png(
    path,
    width = 1900,
...
write_report <- function(path, batch, run_spec, development_summary, artifacts) {
  ...
  lines <- c(
    paste0("# LIT-MR-03.2 ", gsub("_", " ", batch$lane), " Readout"),
    "",
    paste0("Status: `", run_spec$overall_status, "`."),
    "",
    "## Frozen distinction",
    "",
    "- Johansen specification, exact rank one, trade rule, and costs are unchanged.",
    "- Cosine is 0.80; half-life is 2-90; support is 24 with 8 each direction.",
    "- Return q10 and convergence q90 remain mandatory.",
    if (identical(batch$lane, "RETROSPECTIVE")) {
      ...
      paste(
        ...
        "only the first full pass can receive one frozen-vector DEVELOPMENT replay."
      )
    },
    "",
    "## TRAIN",
    "",
    paste0("- Triplets: `", nrow(batch$summary), "`."),
    paste0(
      "- Strict full passes: `",
    ...
    paste0(
      "- Relaxed pass IDs: `",
      if (nrow(passes)) paste(passes$triplet_id, collapse = ", ") else "NONE",
      "`."
    ),
    "",
    "## DEVELOPMENT",
...
lane <- toupper(env_or("GEN5_MR032_LANE", "RETROSPECTIVE"))
if (!lane %in% c("RETROSPECTIVE", "FRESH_ATLAS_01")) {
  stop("GEN5_MR032_LANE must be RETROSPECTIVE or FRESH_ATLAS_01.", call. = FALSE)
}
contract <- g5_mr032_contract()
registry <- g5_mr032_registry_for_lane(lane)
cfg <- g5_load_data_layer_config(repo_root)
cfg$feed <- env_or("GEN5_MR032_FEED", as.character(cfg$feed))
refresh <- env_bool("GEN5_MR032_REFRESH", FALSE)
development_refresh <- env_bool("GEN5_MR032_DEVELOPMENT_REFRESH", refresh)
run_id <- env_or(
  "GEN5_MR032_RUN_ID",
  paste0("lit_mr_03_2_", tolower(lane), "_20260729")
)
...
message("LIT-MR-03.2 ", lane, " TRAIN starting.")
train_query <- g5_workbench_query_adjusted_daily_bars(
  ...
  symbols = g5_mr032_required_symbols(registry, lane),
  universe_name = paste0("lit_mr_03_2_", tolower(lane), "_train"),
...
coverage <- do.call(rbind, lapply(
  g5_mr032_required_symbols(registry, lane),
  function(symbol) {
    observed <- sort(unique(
      train_query$bars$session_date[train_query$bars$symbol == symbol]
    ))
...
if (!identical(analysis_health, "PASS")) {
  stop(
    paste0(
      "TRAIN health failed. Health=", health_max, "."
    ),
    call. = FALSE
  )
}
candidate_coverage <- stats::setNames(
  vapply(seq_len(nrow(registry)), function(i) {
    symbols <- as.character(
      registry[i, c("symbol_1", "symbol_2", "symbol_3")]
    )
    if (all(
      coverage$status[match(symbols, coverage$symbol)] == "PASS"
    )) "PASS" else "FAIL"
  }, character(1)),
  registry$triplet_id
)
batch <- g5_mr032_run_train_batch(
  train_query$bars,
  registry,
  lane,
  data_health_status = analysis_health,
  coverage_status = candidate_coverage
)

development_ids <- if (identical(lane, "RETROSPECTIVE")) {
  batch$relaxed_pass_ids
} else if (!is.na(batch$nominated_triplet_id)) {
  batch$nominated_triplet_id
} else {
  character()
}
...
if (length(development_ids)) {
  development_symbols <- sort(unique(unlist(lapply(development_ids, function(id) {
    row <- registry[registry$triplet_id == id, , drop = FALSE]
    as.character(row[c("symbol_1", "symbol_2", "symbol_3")])
  }))))
  development_query <- g5_workbench_query_adjusted_daily_bars(
    ...
    universe_name = paste0("lit_mr_03_2_", tolower(lane), "_development"),
  ...
  development_reference <- sort(unique(development_query$bars$session_date))
  development_coverage <- do.call(rbind, lapply(
    development_symbols,
    function(symbol) {
      observed <- sort(unique(
        development_query$bars$session_date[
          development_query$bars$symbol == symbol
        ]
      ))
      missing <- setdiff(development_reference, observed)
      data.frame(
        symbol = symbol,
        missing_sessions = length(missing),
        first_session = if (length(observed)) min(observed) else as.Date(NA),
        ...
        status = ifelse(
          length(observed) &&
            length(missing) == 0L &&
            min(observed) <= contract$train_start &&
            max(observed) >= contract$development_end,
          "PASS",
          "FAIL"
        ),
        stringsAsFactors = FALSE
  ...
  development_query_artifacts <- g5_write_workbench_query_artifacts(
    development_query, output_dir, "mr032_development_workbench_query"
  )
  ...
  batch$overall_status <- "RETROSPECTIVE_DESCRIPTIVE_COMPLETE_LIT_MR_03_2"
}

artifacts <- list(
  run_spec_csv = file.path(output_dir, "mr032_run_spec.csv"),
  registry_csv = file.path(output_dir, "mr032_registry.csv"),
  coverage_csv = file.path(output_dir, "mr032_train_coverage.csv"),
  train_summary_csv = file.path(output_dir, "mr032_train_summary.csv"),
  strict_gates_csv = file.path(output_dir, "mr032_strict_gate_detail.csv"),
  relaxed_gates_csv = file.path(output_dir, "mr032_relaxed_gate_detail.csv"),
  development_summary_csv = file.path(output_dir, "mr032_development_summary.csv"),
  development_trades_csv = file.path(output_dir, "mr032_development_trades.csv"),
  development_replay_csv = file.path(output_dir, "mr032_development_replay.csv"),
  report_md = file.path(output_dir, "mr032_report.md"),
  gate_yield_png = file.path(visual_dir, "mr032_gate_yield.png"),
  evidence_plane_png = file.path(visual_dir, "mr032_train_evidence_plane.png"),
  development_png = file.path(visual_dir, "mr032_development_cost_comparison.png")
)
...
development_trades <- bind_nonempty(development_results, function(x) {
  out <- x$trades
  out$triplet_id <- x$summary$triplet_id[[1L]]
  out
})
...
run_spec <- data.frame(
  ...
  train_coverage_status = if (
    all(candidate_coverage == "PASS")
  ) "PASS" else "PARTIAL_CANDIDATE_COVERAGE",
  coverage_ineligible_triplets = sum(candidate_coverage != "PASS"),
...
train_query_artifacts <- g5_write_workbench_query_artifacts(
  train_query, output_dir, "mr032_train_workbench_query"
)
...
message("LIT-MR-03.2 complete: ", batch$overall_status)
