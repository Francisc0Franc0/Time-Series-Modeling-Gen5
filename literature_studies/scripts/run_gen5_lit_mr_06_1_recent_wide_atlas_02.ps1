param(
  [string]$RunId = "lit_mr_06_1_recent_wide_atlas_02_20260730",
  [switch]$RefreshDaily,
...
$env:GEN5_LIT_MR_06_1_ATLAS_ID = "RECENT_WIDE_ATLAS_02"
...
$seedDaily = Join-Path $repoRoot (
  "runs\research_workbench\literature_grounded\" +
  "lit_mr_06_1_recent_wide_atlas_02_registry\candidate_daily_bars.rds"
)
$env:GEN5_LIT_MR_06_1_SEED_DAILY_RDS = if (Test-Path $seedDaily) {
  $seedDaily
} else {
  ""
}
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "LIT-MR-06.1 recent-wide runner failed with exit code $LASTEXITCODE."
  }
}
finally {
  Pop-Location
