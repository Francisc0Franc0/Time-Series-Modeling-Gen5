...
| `LIT-MOM-03.1` | Active Dual Momentum GTAA Replication | none | `RETROSPECTIVE_VALUE_ADD_REPLAY_COMPLETE_NO_ROBUSTNESS_OR_FORWARD_AUTHORITY`; 507 causal weekly intervals, 12.17% net CAGR / -15.45% max drawdown, virtually identical to relative-only, below SPY's 14.36% CAGR, publisher's 2008-2015 span unavailable |
| `LIT-MOM-03.2` | Dual-Momentum Universe Transport POCs | none | `UNIVERSE_TRANSPORT_POC_COMPLETE_DESCRIPTIVE_ONLY`; one clean ten-sector-ETF fleet plus eleven static eight-stock sector fleets, source beat equal-weight CAGR in 1/11 stock sectors and lost by 3.45 points in sector ETFs, defensive drawdown tradeoff rather than broad return transport |
| `LIT-MOM-03.3` | Broad Cross-Sector Stock Fleet POC | none | `BROAD_CROSS_SECTOR_STOCK_FLEET_POC_COMPLETE_DESCRIPTIVE_ONLY`; frozen 29-of-88 sleeves, relative-only beat equal-weight 88 by 0.90 CAGR points, full source lagged equal weight by 0.35 points while improving drawdown; static survivor-biased evidence only |
| `LIT-MOM-03.4` | Defensible 2020 Deployment-Cohort Replay | none | `DEFENSIBLE_UNIVERSE_REPLAY_COMPLETE_STOP_RANKING_EDGE_NOT_NOMINATED`; frozen 481-stock pre-2021 cohort, 271 later weekly intervals, relative-only beat equal weight by only 0.14 CAGR points, source lagged by 1.27 points, SPY led return; no tuning or promotion |
...
- `LIT-REG-02.x` is the swing-horizon directional-state track. `02.1` used a
  package-native two-state Markov-switching AR(1), TRAIN-earned directional
  labels, and causal 20-session positive-return probabilities on planted
  synthetic processes. It stopped after Stage A: all fits and proper forecast
  score gates passed, while hard state-accuracy and transition-tail gates
  failed. No market or strategy data were read.

The tracks must validate independently before any combined direction-by-
volatility regime grid is discussed. `02.2` is forecast-first and preserves
the `02.1` STOP; it does not open market evidence, strategy contact, or a
combined regime grid.
...
| `LIT-MOM-01.2 / AUDIT_01_REPRESENTATIVE_TRADE_TAPES` | Outcome-aware but deterministic visual sample of eight contrasting paths, frozen before individual tape inspection | `RETROSPECTIVE_DESCRIPTIVE_TRADE_TAPE_REVIEW_COMPLETE`; not frequency, asset-selection, filter, or confirmation evidence; Audit 01 STOP preserved |
