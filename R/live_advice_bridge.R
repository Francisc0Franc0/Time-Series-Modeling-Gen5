...
g5_bridge_score_authority_symbol <- function(bars, authority, symbol, as_of_date) {
  ...
  feature_cols <- if ("pca_feature_cols" %in% names(contract) && nzchar(as.character(contract$pca_feature_cols[[1L]]))) {
    trimws(strsplit(as.character(contract$pca_feature_cols[[1L]]), ",", fixed = TRUE)[[1L]])
  } else {
