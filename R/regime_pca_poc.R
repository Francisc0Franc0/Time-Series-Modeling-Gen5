...
g5_pca_regime_relative_features <- function() {
  c(
    "rel_ret_log_21_ctx",
    "rel_ret_log_63_ctx",
    "rel_vol_20_ctx",
...
g5_pca_regime_feature_set <- function(feature_set_id = "current_features_control") {
  feature_set_id <- as.character(feature_set_id)[[1L]]
  control <- g5_pca_regime_default_features()
  trend <- c(
    "ret_log_21",
    "ret_log_63",
    "ret_log_126",
  ...
  relative <- g5_pca_regime_relative_features()
  sets <- list(
    current_features_control = control,
    trend_participation_plus = unique(c(control, trend)),
    trend_volatility_plus = unique(c(control, trend, volatility)),
    trend_volatility_relative_plus = unique(c(control, trend, volatility, relative))
  )
  if (!feature_set_id %in% names(sets)) {
    g5_stop(paste0("Unknown PCA feature_set_id: ", feature_set_id))
  }
  sets[[feature_set_id]]
}
...
g5_pca_regime_feature_set_taxonomy <- function() {
  data.frame(
    ...
    mechanism = c(
      "Existing Gen5.2 PCA feature surface.",
      "Adds participation features for trend persistence and distance from recent highs.",
      "Adds trend plus volatility, downside-risk, range, and drawdown descriptors.",
      "Adds trend/volatility plus cross-sectional context-relative strength and fragility."
    ),
    why = c(
      "Control condition: protects against mistaking feature search for improvement.",
      "Tests whether PCA states under-participate because they do not see enough trend/continuation structure.",
      "Tests whether trend is only useful when separated from unstable or damaged volatility regimes.",
      "Tests whether PCA can distinguish true asset leadership from broad beta exposure."
    ),
    feature_cols = vapply(
      c(
        "current_features_control",
...
g5_pca_regime_lag_ratio <- function(x, n, log = FALSE) {
  x <- as.numeric(x)
  n <- as.integer(n)
  out <- rep(NA_real_, length(x))
  if (length(x) <= n) {
  ...
  prior <- c(rep(NA_real_, n), head(x, -n))
  ratio <- x / prior
  if (isTRUE(log)) ratio <- log(ratio)
  ratio[!is.finite(ratio)] <- NA_real_
  ratio
}

g5_pca_regime_downside_vol <- function(returns, n) {
  returns <- as.numeric(returns)
  n <- as.integer(n)
  if (is.na(n) || n < 2L) {
    g5_stop("downside volatility period must be >= 2.")
  }
  out <- rep(NA_real_, length(returns))
  if (length(returns) < n) {
    return(out)
  }
  for (i in n:length(returns)) {
    window <- returns[(i - n + 1L):i]
    if (all(is.finite(window))) {
      downside <- pmin(window, 0)
      out[[i]] <- sqrt(mean(downside^2))
    }
  }
  out
}
...
g5_pca_regime_feature_table <- function(bars, symbol, end_date = NULL) {
  ...
  ema_20 <- ema_fast
  ema_30 <- g5_pca_regime_ema(close, 30L)
  ...
  vol_63 <- g5_pca_regime_rolling_sd(ret1, 63L)
  atr_14 <- g5_pca_regime_rolling_mean(g5_pca_regime_true_range(high, low, close), 14L)
  rolling_high_63 <- g5_pca_regime_rolling_max(close, 63L)
  range_pct <- (high - low) / pmax(close, 1e-8)
  bb_mid <- sma_20
  ...
  data.frame(
    ...
    ret1 = ret1,
    ret_log_21 = g5_pca_regime_lag_ratio(close, 21L, log = TRUE),
    ret_log_63 = g5_pca_regime_lag_ratio(close, 63L, log = TRUE),
    ret_log_126 = g5_pca_regime_lag_ratio(close, 126L, log = TRUE),
    ema_gap = ema_fast / ema_slow - 1,
    ema_gap_10_30 = ema_10 / ema_30 - 1,
    trend_slope_5 = (ema_fast / c(rep(NA_real_, 5L), head(ema_fast, -5L)) - 1) / 5,
    ema_slope_20_20 = g5_pca_regime_lag_ratio(ema_20, 20L, log = TRUE) / 20,
    rsi_14 = g5_pca_regime_rsi(close, 14L),
    vol_20 = vol_20,
    downside_vol_20 = g5_pca_regime_downside_vol(ret1, 20L),
    atr_pct = atr_14 / close,
    dist_anchor_200 = close / sma_200 - 1,
    dist_high_63 = close / rolling_high_63 - 1,
    drawdown_63 = close / rolling_high_63 - 1,
    chop_14 = g5_pca_regime_adx(high, low, close, 14L),
    bb_width = (bb_up - bb_dn) / pmax(bb_mid, 1e-8),
    range_pct_20 = g5_pca_regime_rolling_mean(range_pct, 20L),
    vol_ratio_20_63 = vol_20 / pmax(vol_63, 1e-8) - 1,
...
g5_pca_regime_add_context_relative_features <- function(features, context_symbols) {
  if (!is.data.frame(features) || !nrow(features)) {
    return(features)
  }
  context_symbols <- unique(g5_standardize_symbol(context_symbols))
  required <- c("ret_log_21", "ret_log_63", "vol_20", "drawdown_63")
  missing <- setdiff(required, names(features))
  if (length(missing)) {
    g5_stop(paste0("Cannot add context-relative PCA features; missing columns: ", paste(missing, collapse = ",")))
  }
  is_context <- as.character(features$symbol) %in% context_symbols
  dates <- as.character(as.Date(features$session_date))
  context_mean <- function(col) {
    means <- tapply(as.numeric(features[[col]])[is_context], dates[is_context], mean, na.rm = TRUE)
    as.numeric(means[dates])
  }
  features$rel_ret_log_21_ctx <- as.numeric(features$ret_log_21) - context_mean("ret_log_21")
  features$rel_ret_log_63_ctx <- as.numeric(features$ret_log_63) - context_mean("ret_log_63")
  features$rel_vol_20_ctx <- as.numeric(features$vol_20) - context_mean("vol_20")
  features$rel_drawdown_63_ctx <- as.numeric(features$drawdown_63) - context_mean("drawdown_63")
  features
}
...
g5_pca_regime_context_feature_table <- function(
  ...
  if (any(g5_pca_regime_relative_features() %in% feature_cols)) {
    g5_stop("Context-relative PCA features are currently supported only for pooled_asset_day PCA panels.")
  }
...
g5_pca_regime_pooled_feature_table <- function(
  ...
  relative_cols <- intersect(feature_cols, g5_pca_regime_relative_features())
  local_feature_cols <- setdiff(feature_cols, relative_cols)
  parts <- lapply(scoring_symbols, function(symbol) {
    features <- g5_pca_regime_feature_table(bars, symbol, end_date = end_date)
    keep <- c("schema_version", "symbol", "session_date", "open", "high", "low", "close", "volume", local_feature_cols)
  ...
  if (any(g5_pca_regime_relative_features() %in% feature_cols)) {
    out <- g5_pca_regime_add_context_relative_features(out, context_symbols)
  }
  rownames(out) <- NULL
  out$regime_context_symbols <- paste(context_symbols, collapse = ",")
  out$research_candidate_symbol <- target_symbol
