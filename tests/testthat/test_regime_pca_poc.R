...
test_that("PCA feature sets expose trend, volatility, and context-relative columns", {
  ...
  ) %in% taxonomy$feature_set_id))

  trend <- g5_pca_regime_feature_set("trend_participation_plus")
  vol <- g5_pca_regime_feature_set("trend_volatility_plus")
  rel <- g5_pca_regime_feature_set("trend_volatility_relative_plus")

  expect_true(all(c("ret_log_63", "ema_gap_10_30", "dist_high_63") %in% trend))
  expect_true(all(c("downside_vol_20", "drawdown_63", "vol_ratio_20_63") %in% vol))
  expect_true(all(g5_pca_regime_relative_features() %in% rel))
})

test_that("PCA pooled relative features compare asset behavior to context mean", {
  amd <- g5_test_pca_bars(symbol = "AMD")
  nvda <- g5_test_pca_bars(symbol = "NVDA")
  tsla <- g5_test_pca_bars(symbol = "TSLA")
  i <- seq_len(nrow(amd))
  nvda$close <- 80 + 0.25 * i + 5 * sin(i / 17)
  nvda$open <- nvda$close * 0.999
  nvda$high <- nvda$close * 1.01
  nvda$low <- nvda$close * 0.99
  tsla$close <- 120 - 0.04 * i + 6 * cos(i / 19)
  tsla$open <- tsla$close * 1.001
  tsla$high <- tsla$close * 1.01
  tsla$low <- tsla$close * 0.99
  bars <- rbind(amd, nvda, tsla)

  features <- g5_pca_regime_pooled_feature_table(
    bars,
    "AMD",
    c("AMD", "NVDA", "TSLA"),
    feature_cols = g5_pca_regime_feature_set("trend_volatility_relative_plus")
  )

  expect_true(all(g5_pca_regime_relative_features() %in% names(features)))
  finite <- features[stats::complete.cases(features[, g5_pca_regime_relative_features(), drop = FALSE]), , drop = FALSE]
  expect_gt(nrow(finite), 0L)
  latest <- finite[finite$session_date == max(finite$session_date), , drop = FALSE]
  expect_lt(abs(mean(latest$rel_ret_log_63_ctx)), 1e-8)
})
