...
test_that("PCA-routed WFA accepts Gen5.3 pooled relative feature set", {
  bars <- g5_test_pca_wfa_context_bars(n = 760L)
  folds <- data.frame(
    fold_id = "F1",
    fold_no = 1L,
    train_start_date = as.Date("2024-04-01"),
    train_end_date = as.Date("2025-03-31"),
    oos_start_date = as.Date("2025-04-01"),
    oos_end_date = as.Date("2025-05-31"),
    stringsAsFactors = FALSE
  )
  ...
  fit <- g5_pca_wfa_fit_fold_models(
    bars,
    symbol = "AMD",
    folds = folds,
  ...
  feature_rows <- fit$pca_model_contract[fit$pca_model_contract$record_type == "feature", , drop = FALSE]
  expect_true(all(g5_pca_regime_relative_features() %in% feature_rows$feature))
  expect_true(all(c("rel_ret_log_63_ctx", "rel_drawdown_63_ctx") %in% names(fit$pca_scores)))
  expect_equal(unique(fit$pca_scores$pca_panel_mode), "pooled_asset_day")
})
