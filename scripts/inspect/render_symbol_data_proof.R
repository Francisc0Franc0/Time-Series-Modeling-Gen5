...
if (!nzchar(start_env) || !nzchar(end_env)) {
  g5_stop("GEN5_DATA_PROOF_START_DATE and GEN5_DATA_PROOF_END_DATE are required.")
}
start_date <- as.Date(start_env)
end_date <- as.Date(end_env)
if (any(is.na(c(start_date, end_date)))) {
  g5_stop("GEN5_DATA_PROOF_START_DATE or GEN5_DATA_PROOF_END_DATE could not be parsed as a date.")
}
