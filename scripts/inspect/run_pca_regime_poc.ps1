...
$rScript = Join-Path $scriptDir "run_pca_regime_poc.R"
...
$env:GEN5_PCA_REGIME_SYMBOL = $Symbol
$env:GEN5_PCA_REGIME_END_DATE = $EndDate
$env:GEN5_PCA_REGIME_AS_OF_TIMESTAMP = $AsOfTimestamp
$env:GEN5_PCA_REGIME_TRAIN_QUARTERS = [string]$TrainQuarters
$env:GEN5_PCA_REGIME_OOS_QUARTERS = [string]$OosQuarters
$env:GEN5_PCA_REGIME_GRID_N = [string]$GridN
$env:GEN5_PCA_REGIME_WARMUP_DAYS = [string]$WarmupDays
$env:GEN5_PCA_REGIME_REFRESH = if ($Refresh.IsPresent) { "true" } else { "false" }

Push-Location $repoRoot
try {
  & $RscriptPath $rScript
  if ($LASTEXITCODE -ne 0) {
    throw "run_pca_regime_poc.R failed with exit code $LASTEXITCODE."
  }
} finally {
  Pop-Location
}
