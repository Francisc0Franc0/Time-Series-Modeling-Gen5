# Gen5.2 style-diversified live-capital generalization screen.
#
# This screen asks whether Gen5.2 selection/replay mechanics generalize across
# very different live-basket styles under true shared-account accounting.
...
feed <- env_or("GEN5_STYLE_DIV_FEED", as.character(cfg$feed))
if (nzchar(feed)) cfg$feed <- feed

refresh <- g5_parse_bool_env(env_or("GEN5_STYLE_DIV_REFRESH", "false"), default = FALSE)
stamp <- gsub("[^0-9A-Za-z]+", "", env_or("GEN5_STYLE_DIV_STAMP", "20260708"))
output_family <- env_or("GEN5_STYLE_DIV_OUTPUT_FAMILY", "gen52_generalization")
output_prefix <- env_or("GEN5_STYLE_DIV_OUTPUT_PREFIX", "style_diversified_live_capital")
output_dir <- file.path(repo_root, "runs", "research_workbench", output_family, paste0(output_prefix, "_", stamp))
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

run_scope <- env_or("GEN5_STYLE_DIV_SCOPE", "full")
selection_policies <- split_csv(env_or("GEN5_STYLE_DIV_SELECTION_POLICIES", "asset_state_direct_spec,pooled_family_asset_variant"))
entry_replay_semantics <- split_csv(env_or("GEN5_STYLE_DIV_ENTRY_REPLAY_SEMANTICS", "fresh_signal_only,state_switch_continuation"))
feature_set_ids <- split_csv(env_or("GEN5_STYLE_DIV_FEATURE_SETS", "current_features_control"))
feature_sets <- stats::setNames(lapply(feature_set_ids, g5_pca_regime_feature_set), feature_set_ids)

feature_set_slug <- function(feature_set_id) {
  slug <- switch(
    as.character(feature_set_id),
    current_features_control = "ctrl",
    trend_participation_plus = "trend",
    trend_volatility_plus = "trendvol",
    trend_volatility_relative_plus = "trendrel",
    gsub("[^0-9A-Za-z]+", "_", as.character(feature_set_id))
  )
  tolower(gsub("^_+|_+$", "", slug))
}

min_train_state_rows <- 20L
warmup_days <- 420L
grid_n <- 3L
initial_capital <- as.numeric(env_or("GEN5_STYLE_DIV_INITIAL_CAPITAL", "100000"))
strategy_grid_preset <- g5_wfa_strategy_grid_preset(env_or("GEN5_STYLE_DIV_STRATEGY_GRID_PRESET", "gen4_daily_default"))
candidate_families <- g5_wfa_candidate_families(split_csv(env_or(
  "GEN5_STYLE_DIV_CANDIDATE_FAMILIES",
  "ema_cross,ema_trend,bollinger_touch,bollinger_mid_reversion,rsi_mr,zret_mr,breakout,pullback_in_uptrend,vol_expansion_breakout,donchian_breakout_vol_expand,no_trade"
)))
...
broad_context <- c("SPY", "QQQ", "IWM", "TLT", "GLD")
base_screen_specs <- list(
  list(
    screen_id = "HB_uniform_broad_context",
    screen_label = "High beta growth / uniform broad context",
    ...
    interpretation_note = "Volatile growth basket; tests whether the mechanics capture upside without merely becoming buy-and-hold beta."
  ),
  list(
    screen_id = "DEF_uniform_broad_context",
    screen_label = "Defensive staples / uniform broad context",
    basket_archetype = "defensive_staples",
    symbols = unique_symbols(c("KO", "PEP", "WMT", "COST", "XLP")),
    context_symbols = with_context(c("KO", "PEP", "WMT", "COST", "XLP"), broad_context),
    interpretation_note = "Defensive/consumer basket; tests whether the mechanics can adapt when upside is slower and drawdown avoidance matters more."
  ),
  list(
    screen_id = "COM_uniform_broad_context",
    screen_label = "Energy, gold, and metals / uniform broad context",
    basket_archetype = "energy_commodity",
    symbols = unique_symbols(c("XLE", "CVX", "XOM", "GLD", "SLV")),
    context_symbols = with_context(c("XLE", "CVX", "XOM", "GLD", "SLV"), broad_context),
    interpretation_note = "Energy/commodity-linked basket; tests whether the mechanics can handle a different return driver than tech beta."
  )
)

if (identical(run_scope, "diverse_smoke")) {
  smoke_symbols <- list(
    high_beta_growth = unique_symbols(c("AMD", "NVDA")),
    defensive_staples = unique_symbols(c("KO", "WMT")),
    energy_commodity = unique_symbols(c("XLE", "GLD"))
  )
  base_screen_specs <- lapply(base_screen_specs, function(spec) {
    symbols <- smoke_symbols[[spec$basket_archetype]]
    if (!is.null(symbols)) {
      spec$symbols <- symbols
      spec$context_symbols <- with_context(symbols, broad_context)
      spec$interpretation_note <- paste(
        spec$interpretation_note,
        "Diverse smoke scope uses two active symbols per basket to validate the feature hypothesis before full-width compute."
      )
    }
    spec
  })
  windows <- windows[windows$quarter_id == "2022Q1", , drop = FALSE]
}

screen_specs <- unlist(lapply(base_screen_specs, function(spec) {
  lapply(feature_set_ids, function(feature_set_id) {
    out <- spec
    out$base_screen_id <- spec$screen_id
    out$feature_set_slug <- feature_set_slug(feature_set_id)
    out$feature_set_id <- feature_set_id
    out$feature_cols <- feature_sets[[feature_set_id]]
    out$screen_id <- paste(spec$screen_id, out$feature_set_slug, sep = "__")
    out$screen_label <- paste(spec$screen_label, feature_set_id, sep = " / ")
    out$interpretation_note <- paste(spec$interpretation_note, "Feature set:", feature_set_id)
    out
  })
}), recursive = FALSE)

only_screens <- split_csv(env_or("GEN5_STYLE_DIV_ONLY", ""))
if (length(only_screens)) {
  screen_specs <- Filter(function(x) x$screen_id %in% only_screens || x$base_screen_id %in% only_screens, screen_specs)
  if (!length(screen_specs)) g5_stop("GEN5_STYLE_DIV_ONLY did not match any configured screen_id.")
}
window_override <- split_csv(env_or("GEN5_STYLE_DIV_WINDOWS", ""))
if (length(window_override)) {
  ...
  if (!nrow(windows)) g5_stop("GEN5_STYLE_DIV_WINDOWS did not match any configured window.")
}
...
build_authority <- function(spec, bars, quarter_id, authority_dir) {
  ...
  contract$research_note <- "Style-diversified Gen5.2 generalization screen: behavioral-pool PCA, 3x3 quantile states, uniform broad context, live-capital portfolio replay."
  contract$grid_n <- grid_n
  contract$selection_policy <- "base_direct_authority"
  contract$pca_feature_set_id <- spec$feature_set_id
  contract$pca_feature_cols <- paste(spec$feature_cols, collapse = ",")
...
replay_symbol_oos <- function(bars, authority, symbol, as_of_date, entry_semantics, lane_id, screen_id, window_id) {
  ...
  out <- g5_bridge_replay_symbol(
    ...
    authority_role = paste0("style_diversified_", lane_id),
...
summarize_accounting <- function(screen_id, base_screen_id, basket_archetype, feature_set_id, window, lane_id, policy, semantics, accounting, initial_capital) {
...
write_equity_overlay <- function(equity_all, summary, path) {
  ...
  lane_colors <- c(
    asset_state_direct_spec__fresh_signal_only = "#2E86AB",
    asset_state_direct_spec__state_switch_continuation = "#00A88F",
    pooled_family_asset_variant__fresh_signal_only = "#9B5DE5",
    pooled_family_asset_variant__state_switch_continuation = "#FF9F1C"
  )
  grDevices::png(path, width = 3600L, height = max(2600L, 380L * length(screens)), res = 220L)
  ...
  graphics::mtext("Style-Diversified Live-Capital Equity Overlay", side = 3, outer = TRUE, line = 1, font = 2, col = aesthetic$text)
  graphics::legend("bottom", inset = -0.02, legend = c("direct fresh", "direct continuation", "pooled fresh", "pooled continuation", "basket hold", "SPY hold"), col = c(lane_colors, "#111111", "#777777"), lty = c(1, 1, 1, 1, 2, 3), lwd = c(1.8, 1.8, 1.8, 1.8, 1.4, 1.2), horiz = TRUE, bty = "n", cex = 0.82, xpd = NA)
  invisible(path)
}
...
write_alpha_heatmap <- function(summary, path) {
  ...
  grDevices::png(path, width = 2600L, height = max(2300L, 115L * length(rows) + 700L), res = 220L)
  oldpar <- graphics::par(no.readonly = TRUE)
  on.exit({ graphics::par(oldpar); grDevices::dev.off() }, add = TRUE)
  graphics::par(bg = aesthetic$background, mar = c(6, 13, 4, 2))
...
write_alpha_bar_chart <- function(summary, path) {
  aesthetic <- g5_chart_aesthetic()
  basket_label <- c(
    high_beta_growth = "HB",
    defensive_staples = "Defensive",
  ...
  summary$plot_label <- paste(
    basket_label[as.character(summary$basket_archetype)],
    feature_label[as.character(summary$feature_set_id)],
    policy_label[as.character(summary$selection_policy)]
  )
  summary <- summary[order(summary$basket_archetype, summary$feature_set_id, summary$selection_policy), , drop = FALSE]
  ...
  oldpar <- graphics::par(no.readonly = TRUE)
  on.exit({ graphics::par(oldpar); grDevices::dev.off() }, add = TRUE)
  graphics::par(bg = aesthetic$background, mar = c(5, 12, 4, 2))
  xlim <- range(c(alpha, 0), na.rm = TRUE)
  pad <- diff(xlim) * 0.08
  ...
  graphics::text(
    rev(alpha),
    bars,
    labels = pct_label(rev(alpha), 1L),
    pos = ifelse(rev(alpha) >= 0, 4, 2),
    cex = 0.68,
    col = aesthetic$text,
    xpd = NA
  )
...
write_exposure_alpha_scatter <- function(summary, path) {
  aesthetic <- g5_chart_aesthetic()
  colors <- c(high_beta_growth = "#2E86AB", defensive_staples = "#00A88F", energy_commodity = "#FF9F1C")
...
write_report <- function(paths, run_spec, summary, aggregate) {
  ...
  lines <- c(
    ...
    "",
    "This screen deliberately steps away from the SOFI/AMD calibration path. It asks whether the Gen5.2 selection and replay mechanics can produce basket-relative alpha across asset styles that behave differently: high-beta growth, defensive staples, and energy/commodity-linked assets.",
    "",
    "The intent is to avoid mistaking increased exposure to one hot high-beta window for a generalizable regime-conditioned trading edge.",
    "",
    "## Design",
    "",
    "- PCA/state surface: behavioral-pool long PCA plus `3x3` quantile states.",
    "- Context recipe: each basket plus the same broad anchors `SPY,QQQ,IWM,TLT,GLD`; no `VXX`.",
    "- Selection policies: Gen5.2 direct-spec and Gen4-style pooled-family.",
    ...
    "## Run Spec",
    "",
    md_table(run_spec, c("screen_id", "basket_archetype", "feature_set_id", "symbols", "context_symbols", "window_id", "selection_policy", "entry_replay_semantics"), n = 24L),
    "",
    "## Live-Capital Summary",
    "",
    md_table(printable, c("screen_id", "feature_set_id", "window_id", "selection_policy", "entry_replay_semantics", "total_return", "active_equal_buy_hold_return", "alpha_vs_active_equal", "mean_open_position_fraction", "total_entry_fills", "max_drawdown")),
    "",
    "## Aggregate Readout",
    "",
    md_table(agg, c("basket_archetype", "feature_set_id", "selection_policy", "entry_replay_semantics", "windows_tested", "windows_beating_basket", "mean_total_return", "mean_alpha_vs_active_equal", "mean_exposure", "worst_drawdown")),
    "",
    "## Visual Outputs",
    "",
    paste0("- Equity overlay: `", paths$equity_overlay_png, "`"),
    paste0("- Alpha heatmap: `", paths$alpha_heatmap_png, "`"),
    paste0("- Alpha bar chart: `", paths$alpha_bar_png, "`"),
    ...
    "- The fallback calibration lane is intentionally excluded from this first generalization test."
  )
...
for (spec in screen_specs) {
  ...
  query_prefix <- paste0("query_", spec$base_screen_id, "__", spec$feature_set_slug)
  message("")
  ...
  query <- g5_workbench_query_adjusted_daily_bars(
    ...
    universe_name = paste0("gen53feat_", spec$base_screen_id, "_", spec$feature_set_slug),
  ...
  query_paths <- g5_write_workbench_query_artifacts(query, output_dir = query_dir, prefix = query_prefix)
  bars <- query$bars
  ...
  for (w in seq_len(nrow(windows))) {
    ...
    policy_states$base_screen_id <- spec$base_screen_id
    ...
    for (policy in selection_policies) {
      for (semantics in entry_replay_semantics) {
        ...
        eq$base_screen_id <- spec$base_screen_id
        ...
        if (is.data.frame(ev) && nrow(ev)) {
          ...
          ev$base_screen_id <- spec$base_screen_id
        ...
        standalone$base_screen_id <- spec$base_screen_id
        ...
        sym$base_screen_id <- spec$base_screen_id
        ...
        summary_rows[[length(summary_rows) + 1L]] <- summarize_accounting(spec$screen_id, spec$base_screen_id, spec$basket_archetype, spec$feature_set_id, window, lane_id, policy, semantics, accounting, initial_capital)
        ...
        packet_rows[[length(packet_rows) + 1L]] <- data.frame(
          ...
          base_screen_id = spec$base_screen_id,
          basket_archetype = spec$basket_archetype,
          feature_set_slug = spec$feature_set_slug,
...
run_spec <- do.call(rbind, lapply(screen_specs, function(spec) {
  data.frame(
    ...
    feature_set_slug = spec$feature_set_slug,
    feature_set_id = spec$feature_set_id,
    pca_feature_cols = paste(spec$feature_cols, collapse = ","),
    symbols = paste(spec$symbols, collapse = ","),
    context_symbols = paste(spec$context_symbols, collapse = ","),
    context_recipe = "live_basket_plus_uniform_broad_anchors_no_vxx",
    pca_panel_mode = "pooled_asset_day",
    run_scope = run_scope,
    state_engine = "quantile_grid",
    grid_n = grid_n,
...
aggregate <- do.call(rbind, lapply(split(summary, paste(summary$basket_archetype, summary$feature_set_id, summary$selection_policy, summary$entry_replay_semantics, sep = "|")), function(x) {
  data.frame(
...
aggregate <- aggregate[order(aggregate$basket_archetype, aggregate$feature_set_id, aggregate$selection_policy, aggregate$entry_replay_semantics), , drop = FALSE]

paths <- list(
  run_spec_csv = file.path(output_dir, "style_diversified_live_capital_run_spec.csv"),
  packet_index_csv = file.path(output_dir, "style_diversified_live_capital_packet_index.csv"),
  selected_states_csv = file.path(output_dir, "style_diversified_live_capital_selected_states.csv"),
  replay_oos_csv = file.path(output_dir, "style_diversified_live_capital_replay_oos.csv"),
  executions_csv = file.path(output_dir, "style_diversified_live_capital_executions.csv"),
  trades_csv = file.path(output_dir, "style_diversified_live_capital_trades.csv"),
  pending_csv = file.path(output_dir, "style_diversified_live_capital_pending_actions.csv"),
  portfolio_equity_csv = file.path(output_dir, "style_diversified_live_capital_portfolio_equity.csv"),
  portfolio_events_csv = file.path(output_dir, "style_diversified_live_capital_portfolio_events.csv"),
  standalone_symbol_equity_csv = file.path(output_dir, "style_diversified_live_capital_standalone_symbol_equity.csv"),
  symbol_summary_csv = file.path(output_dir, "style_diversified_live_capital_symbol_summary.csv"),
  summary_csv = file.path(output_dir, "style_diversified_live_capital_summary.csv"),
  aggregate_csv = file.path(output_dir, "style_diversified_live_capital_aggregate.csv"),
  equity_overlay_png = file.path(output_dir, "style_diversified_live_capital_equity_overlay.png"),
  alpha_heatmap_png = file.path(output_dir, "style_diversified_live_capital_alpha_heatmap.png"),
  alpha_bar_png = file.path(output_dir, "style_diversified_live_capital_alpha_bar.png"),
  exposure_alpha_scatter_png = file.path(output_dir, "style_diversified_live_capital_exposure_alpha_scatter.png"),
  artifact_index_csv = file.path(output_dir, "style_diversified_live_capital_artifact_index.csv"),
  report_md = file.path(output_dir, "style_diversified_live_capital_report.md")
)
...
write_alpha_bar_chart(summary, paths$alpha_bar_png)
...
message("")
message("Style-diversified live-capital screen complete: ", normalizePath(output_dir, winslash = "/", mustWork = FALSE))
message("Summary:")
print(printable[, c("screen_id", "feature_set_id", "window_id", "selection_policy", "entry_replay_semantics", "total_return", "active_equal_buy_hold_return", "alpha_vs_active_equal", "mean_open_position_fraction", "total_entry_fills"), drop = FALSE], row.names = FALSE)
message("")
message("Report: ", paths$report_md)
message("Deck visuals: ", paths$equity_overlay_png, " / ", paths$alpha_heatmap_png, " / ", paths$exposure_alpha_scatter_png)
