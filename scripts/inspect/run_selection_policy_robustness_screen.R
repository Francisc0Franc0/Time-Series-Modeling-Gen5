# Broader selection-policy robustness screen.
...
refresh <- g5_parse_bool_env(env_or("GEN5_SELECTION_POLICY_REFRESH", "false"), default = FALSE)
stamp <- gsub("[^0-9A-Za-z]+", "", env_or("GEN5_SELECTION_POLICY_SCREEN_STAMP", "20260702"))
root_output_dir <- file.path(repo_root, "runs", "research_workbench", "selection_policy_screens", paste0("selpol_robust_", stamp))
...
screen_specs <- list(
  list(
    screen_id = "A_live",
    screen_label = "Screen A: current live basket, max SOFI/PLTR history",
    evidence_role = "live_basket_evidence",
    symbols = c("AMD", "NVDA", "PLTR", "TSLA", "SOFI"),
    context_symbols = gen4_context,
    ...
    interpretation_note = "This is the direct apples-to-apples screen for the current Gen4-like live basket. History is limited by SOFI, PLTR, and several Gen4 context symbols, so the clean replay span starts after an 8-quarter TRAIN seed."
  ),
  list(
    screen_id = "B_hist",
    screen_label = "Screen B: historical substitute basket",
    evidence_role = "substitute_basket_robustness_evidence",
    symbols = c("AMD", "NVDA", "TSLA", "AAPL", "MSTR"),
    context_symbols = c("AMD", "NVDA", "TSLA", "AAPL", "MSTR", "SPY", "QQQ", "IWM", "SMH", "TLT", "GLD"),
    authority_quarters = c("2018Q4", "2019Q1", "2020Q2", "2020Q3", "2021Q4", "2022Q1", "2022Q3", "2022Q4", "2024Q4", "2025Q1"),
    replay_windows = data.frame(
      window_id = c("2019Q1_asof_20190329", "2020Q3_asof_20200930", "2022Q1_asof_20220331", "2022Q4_asof_20221230", "2025Q1_asof_20250331"),
      quarter_id = c("2019Q1", "2020Q3", "2022Q1", "2022Q4", "2025Q1"),
      as_of_timestamp = c("2019-03-29 17:30:00", "2020-09-30 17:30:00", "2022-03-31 17:30:00", "2022-12-30 17:30:00", "2025-03-31 17:30:00"),
      stringsAsFactors = FALSE
    ),
    interpretation_note = "This screen swaps out SOFI and PLTR so the policy comparison can span older regimes. It is not a live-basket replication; it is robustness evidence about the selection-policy fork under a long-history volatile-growth basket."
  )
)

only_screens <- strsplit(env_or("GEN5_SELECTION_POLICY_ONLY", ""), ",", fixed = TRUE)[[1L]]
...
if (length(only_screens)) {
  ...
  if (!length(screen_specs)) g5_stop("GEN5_SELECTION_POLICY_ONLY did not match any configured screen_id.")
}
...
get_or_build_authority <- function(spec, bars, screen_dir, quarter_id) {
  ...
  if (file.exists(file.path(authority_dir, "bridge_authority_contract.csv")) &&
...
run_screen <- function(spec) {
  ...
  report <- c(
    ...
    "",
    "This screen tests a narrow architectural question: after pooled PCA creates shared market states, should Gen5.1 choose the best full strategy separately for each asset/state, or should it first choose the best strategy family for the state from pooled TRAIN evidence and only then choose asset-level parameters?",
    "",
    "The purpose is not to crown a live allocation. It is to see whether the policy fork changes behavior consistently across more than one recent quarter, while holding data, PCA, state map, strategy grid, replay mechanics, and continuity rules fixed within each screen.",
    "",
    "## Scope Note",
    "",
...
message("Gen5.1 selection-policy robustness screen")
...
master_paths <- list(
  run_spec_csv = file.path(root_output_dir, "selection_policy_robustness_run_spec.csv"),
  agreement_summary_csv = file.path(root_output_dir, "selection_policy_robustness_agreement_summary.csv"),
  portfolio_proxy_csv = file.path(root_output_dir, "selection_policy_robustness_portfolio_proxy_summary.csv"),
  trade_summary_csv = file.path(root_output_dir, "selection_policy_robustness_trade_summary.csv"),
  packet_index_csv = file.path(root_output_dir, "selection_policy_robustness_packet_index.csv"),
  report_md = file.path(root_output_dir, "selection_policy_robustness_report.md")
)
...
report <- c(
  "# Gen5.1 Selection-Policy Robustness Packet",
  "",
  "## Purpose",
  "",
  "This packet broadens the Gen4-style pooled-family versus current Gen5.1 direct-spec selection comparison. The goal is to avoid deciding from one recent slice by separating live-basket evidence from longer-history substitute-basket evidence.",
  "",
  "## Screens",
  "",
  md_table(master_run_spec, c("screen_id", "evidence_role", "symbols", "authority_quarters", "replay_windows")),
  "",
  "## Agreement Summary",
  "",
  ...
  "",
  "Screen A is the true current live-basket comparison. Screen B is longer-history robustness evidence created by replacing newer-history assets with older-history substitutes. Do not merge them into one winner table without preserving that distinction.",
  "",
  "## Artifact Roots",
  "",
  ...
  "## STOP Decisions",
  "",
  "- Whether the live bridge should stay on direct full-spec selection or add pooled-family selection as an operator-approved mode.",
  "- Whether future context-universe/state-map factorial screens should include selection policy as a declared factor.",
  "- Whether Screen B substitute-basket evidence is strong enough to motivate older-history reruns with other basket substitutions."
)
writeLines(unlist(report), master_paths$report_md, useBytes = TRUE)

message("")
message("Robustness screen complete:")
