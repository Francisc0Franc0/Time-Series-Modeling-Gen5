...
pca_panel_mode <- env_or("GEN5_PCA_WFA_PANEL_MODE", "date_aligned_context")
...
if (!pca_panel_mode %in% c("date_aligned_context", "pooled_asset_day")) g5_stop("GEN5_PCA_WFA_PANEL_MODE must be date_aligned_context or pooled_asset_day.")
...
strategy_grid_preset <- g5_wfa_strategy_grid_preset(env_or("GEN5_PCA_WFA_STRATEGY_GRID_PRESET", "gen4_daily_default"))
candidate_families <- g5_wfa_candidate_families(parse_character_list("GEN5_PCA_WFA_CANDIDATE_FAMILIES", "ema_cross,ema_trend,bollinger_touch,bollinger_mid_reversion,rsi_mr,zret_mr,breakout,pullback_in_uptrend,vol_expansion_breakout,donchian_breakout_vol_expand,no_trade"))
...
message(if (identical(state_engine, "pca_kmeans_auto")) paste0("PCA auto k-means max clusters: ", grid_n) else if (identical(state_engine, "pca_kmeans")) paste0("PCA k-means clusters: ", grid_n) else paste0("PCA grid: ", grid_n, "x", grid_n))
