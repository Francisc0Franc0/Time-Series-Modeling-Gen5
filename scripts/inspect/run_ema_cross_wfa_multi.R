# Gen5.1 three-fold EMA cross WFA proof of concept.
...
if (!nzchar(as_of_env)) {
  g5_stop("GEN5_AS_OF_TIMESTAMP is required for EMA multi-fold WFA.")
}
...
if (!nzchar(symbol_env)) {
  g5_stop("GEN5_WFA_MULTI_SYMBOL is required for EMA multi-fold WFA.")
}
...
if (wfa_start_date > wfa_end_date) {
  g5_stop("EMA multi-fold WFA start date cannot be after end date.")
}
...
result <- g5_workbench_query_adjusted_daily_bars(
  ...
  symbols = symbol,
  universe_name = paste0("ema_cross_wfa_multi_", symbol),
