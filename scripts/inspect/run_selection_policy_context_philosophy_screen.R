# Selection-policy x context-philosophy research screen.
...
refresh <- g5_parse_bool_env(env_or("GEN5_SELECTION_POLICY_CONTEXT_REFRESH", "false"), default = FALSE)
stamp <- gsub("[^0-9A-Za-z]+", "", env_or("GEN5_SELECTION_POLICY_CONTEXT_STAMP", "20260703"))
root_output_dir <- file.path(repo_root, "runs", "research_workbench", "selpol_context", paste0("selpol_context_", stamp))
...
high_beta_symbols <- c("AMD", "NVDA", "TSLA", "AAPL", "MSTR")
etf_sector_symbols <- c("QQQ", "SMH", "XLK", "XLE", "XLF")

broad_risk_add_on_high_beta <- c("SPY", "QQQ", "IWM", "SMH", "TLT", "GLD")
broad_risk_add_on_etf <- c("SPY", "IWM", "TLT", "GLD")
archetype_add_on_high_beta <- c("AVGO", "MU", "QCOM", "NFLX", "META", "AMZN", "SHOP", "LRCX", "PYPL", "SMCI")
archetype_add_on_etf <- c("VGT", "IYW", "SOXX", "XLI", "XLY", "XLV", "XLP", "XLU", "XBI")
diverse_behavior_add_on <- c("SPY", "QQQ", "IWM", "SMH", "TLT", "GLD", "AVGO", "MU", "QCOM", "NFLX", "META", "AMZN", "SHOP", "LRCX", "PYPL", "SMCI", "VGT", "IYW", "SOXX", "XLI", "XLY", "XLV", "XLP", "XLU", "XBI")
size_matched_diverse_add_on <- c("SPY", "TLT", "GLD", "AVGO", "META", "AMZN", "VGT", "SOXX", "XBI")

make_screen_spec <- function(screen_id, screen_label, basket_archetype, symbols, context_philosophy, context_symbols, note) {
  list(
    screen_id = screen_id,
    screen_label = screen_label,
    evidence_role = paste0(basket_archetype, "_", context_philosophy, "_evidence"),
    symbols = unique_symbols(symbols),
    context_symbols = unique_symbols(context_symbols),
    basket_archetype = basket_archetype,
    context_philosophy = context_philosophy,
    authority_quarters = authority_quarters,
...
screen_specs <- list(
  make_screen_spec(
    ...
    "High beta / broad-risk no VXX",
    "long_history_high_beta_growth",
    high_beta_symbols,
    "broad_risk_no_vxx",
    with_context(high_beta_symbols, broad_risk_add_on_high_beta),
    "Long-history high-beta basket with generic market/rates/gold/sector anchors. This is the cleaned companion to the prior broad-risk pilot, with VXX removed from behavioral-pool PCA."
  ),
  make_screen_spec(
    "HB_archetype_matched_no_vxx",
    "High beta / archetype-matched no VXX",
    "long_history_high_beta_growth",
    high_beta_symbols,
    "archetype_matched_no_vxx",
    with_context(high_beta_symbols, archetype_add_on_high_beta),
    "Long-history high-beta basket with volatile growth, semiconductor, and mega-cap tech peers as behavioral context."
  ),
  make_screen_spec(
    "HB_diverse_behavior_no_vxx",
    "High beta / diverse-behavior no VXX",
    "long_history_high_beta_growth",
    high_beta_symbols,
    "diverse_behavior_no_vxx",
    with_context(high_beta_symbols, diverse_behavior_add_on),
    "Long-history high-beta basket with a large mixed behavioral library: market anchors, rates/gold, high-beta peers, and sector ETFs."
  ),
  make_screen_spec(
    "HB_size_matched_diverse_no_vxx",
    "High beta / size-matched diverse no VXX",
    "long_history_high_beta_growth",
    high_beta_symbols,
    "size_matched_diverse_no_vxx",
    with_context(high_beta_symbols, size_matched_diverse_add_on),
    "Long-history high-beta basket with a smaller balanced mixed context so diversity is tested without simply adding every available behavior type."
  ),
  make_screen_spec(
    "ETF_broad_risk_no_vxx",
    "ETF-sector / broad-risk no VXX",
    "etf_sector_tradeable_proxy",
    etf_sector_symbols,
    "broad_risk_no_vxx",
    with_context(etf_sector_symbols, broad_risk_add_on_etf),
    "ETF/sector basket with generic market/rates/gold anchors and no VXX."
  ),
  make_screen_spec(
    "ETF_archetype_matched_no_vxx",
    "ETF-sector / archetype-matched no VXX",
    "etf_sector_tradeable_proxy",
    etf_sector_symbols,
    "archetype_matched_no_vxx",
    with_context(etf_sector_symbols, archetype_add_on_etf),
    "ETF/sector basket with ETF and sector peers as behavioral context."
  ),
  make_screen_spec(
    "ETF_diverse_behavior_no_vxx",
    "ETF-sector / diverse-behavior no VXX",
    "etf_sector_tradeable_proxy",
    etf_sector_symbols,
    "diverse_behavior_no_vxx",
    with_context(etf_sector_symbols, diverse_behavior_add_on),
    "ETF/sector basket with a large mixed behavioral library: market anchors, rates/gold, high-beta peers, and sector ETFs."
  ),
  make_screen_spec(
    "ETF_size_matched_diverse_no_vxx",
    "ETF-sector / size-matched diverse no VXX",
    "etf_sector_tradeable_proxy",
    etf_sector_symbols,
    "size_matched_diverse_no_vxx",
    with_context(etf_sector_symbols, size_matched_diverse_add_on),
    "ETF/sector basket with a smaller balanced mixed context so diversity is tested without simply adding every available behavior type."
  )
)

only_screens <- strsplit(env_or("GEN5_SELECTION_POLICY_CONTEXT_ONLY", ""), ",", fixed = TRUE)[[1L]]
...
if (length(only_screens)) {
  ...
  if (!length(screen_specs)) g5_stop("GEN5_SELECTION_POLICY_CONTEXT_ONLY did not match any configured screen_id.")
}
...
run_screen <- function(spec) {
  ...
  agreement_by_quarter <- do.call(rbind, lapply(split(comparison, comparison$quarter_id), function(x) {
    data.frame(
      ...
      context_philosophy = spec$context_philosophy,
    ...
    for (policy in selection_policies) {
      ...
      for (field in c("replay", "pending_actions", "executions", "trades", "operator_packet", "book_summary", "continuity")) {
        if (!is.null(daily[[field]]) && is.data.frame(daily[[field]]) && nrow(daily[[field]])) {
          ...
          daily[[field]]$basket_archetype <- spec$basket_archetype
          daily[[field]]$context_philosophy <- spec$context_philosophy
        }
      }
      ...
      summary$basket_archetype <- spec$basket_archetype
      summary$context_philosophy <- spec$context_philosophy
  ...
  run_spec <- data.frame(
    ...
    context_size = length(spec$context_symbols),
    context_add_on_symbols = paste(setdiff(spec$context_symbols, spec$symbols), collapse = ","),
    ...
    context_history_policy = "no_vxx_behavioral_pool_context",
    min_train_state_rows = min_train_state_rows,
    intended_variable = "selection_policy_x_context_philosophy_x_basket_archetype",
    research_only = TRUE,
  ...
  report <- c(
    paste0("# Gen5.1 Selection-Policy x Context-Philosophy Screen: ", spec$screen_label),
    "",
    "## Plain-Language Purpose",
    "",
    "This screen tests a narrow architectural question across a declared basket archetype and context philosophy: after pooled PCA creates shared market states, should Gen5.1 choose the best full strategy separately for each asset/state, or should it first choose the best strategy family for the state from pooled TRAIN evidence and only then choose asset-level parameters?",
    "",
    "The purpose is not to crown a live allocation. It is to see whether the policy fork changes behavior consistently across basket types, context-construction philosophies, and market windows, while holding PCA, state map, strategy grid, replay mechanics, and continuity rules fixed within each screen.",
    "",
    "## Scope Note",
    "",
    ...
    paste0("- Context philosophy: `", spec$context_philosophy, "`"),
...
message("Gen5.1 selection-policy x context-philosophy screen")
...
master_paths <- list(
  run_spec_csv = file.path(root_output_dir, "selection_policy_context_philosophy_run_spec.csv"),
  agreement_summary_csv = file.path(root_output_dir, "selection_policy_context_philosophy_agreement_summary.csv"),
  portfolio_proxy_csv = file.path(root_output_dir, "selection_policy_context_philosophy_portfolio_proxy_summary.csv"),
  trade_summary_csv = file.path(root_output_dir, "selection_policy_context_philosophy_trade_summary.csv"),
  packet_index_csv = file.path(root_output_dir, "selection_policy_context_philosophy_packet_index.csv"),
  report_md = file.path(root_output_dir, "selection_policy_context_philosophy_report.md")
)
...
report <- c(
  "# Gen5.1 Selection-Policy x Context-Philosophy Packet",
  "",
  "## Purpose",
  "",
  "This packet broadens the Gen4-style pooled-family versus direct-spec selection comparison by treating context-universe construction as an explicit research factor. The goal is to learn whether selection policy and context design interact across long-history high-beta stocks and liquid ETF/sector proxies.",
  "",
  "The immediate setup supersedes the prior basket-archetype pilot rather than deleting it. That earlier packet was useful, but it mixed a recent SOFI/PLTR live-like lane with VXX-inclusive behavioral-pool context. This screen removes VXX from behavioral-pool PCA, drops the SOFI-dependent lane to preserve history and compute, and compares cleaner context philosophies on the same historical windows.",
  "",
  "## Screens",
  "",
  md_table(master_run_spec, c("screen_id", "evidence_role", "basket_archetype", "context_philosophy", "context_size", "symbols", "authority_quarters", "replay_windows")),
  "",
  "## Agreement Summary",
  "",
  ...
  "",
  md_table(master_portfolio, c("screen_id", "basket_archetype", "context_philosophy", "window_id", "selection_policy", "trade_count", "open_trade_count", "equal_symbol_mean_compound_trace_return", "worst_symbol_compound_trace_return", "best_symbol_compound_trace_return")),
  "",
  "## Interpretation Guardrail",
  "",
  "These lanes are generalization probes, not production allocation evidence. Do not collapse them into a single winner table without preserving basket archetype, context philosophy, and selection policy. The large diverse context lane also has a size confound by design; the size-matched diverse lane exists to make that visible.",
  "",
  "## Artifact Roots",
  "",
  ...
  "- Whether active_plus_risk-style context should be replaced, narrowed, or expanded by style-matched or diverse context construction.",
  "- Whether high-beta and ETF/sector baskets should share one context recipe or use basket-specific context recipes.",
  "- Whether context diversity is useful after controlling for context size."
)
writeLines(unlist(report), master_paths$report_md, useBytes = TRUE)

message("")
message("Context-philosophy screen complete:")
