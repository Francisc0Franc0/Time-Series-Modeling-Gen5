# Frozen HYP-MOM-07.1 QQQ / equal-weight leadership helpers.

g5_hm071_stop <- function(message) stop(message, call. = FALSE)

g5_hm071_schema_version <- function() "gen5_hyp_mom_07_1_v1"

g5_hm071_contract <- function() {
  list(
    hypothesis_id = "HYP-MOM-07.1",
    descriptive_name = "QQQ / Equal-Weight Leadership Persistence",
    symbols = c("QQQ", "QQEW"),
    cap_symbol = "QQQ",
    equal_symbol = "QQEW",
    ...
    confirmation_end = as.Date("2025-12-19"),
    qqew_mandate_change = as.Date("2025-12-22"),
    ...
    minimum_zone_anchors = 500L
  )
}
g5_hm071_validate_contract <- function(contract = g5_hm071_contract()) {
  frozen <- g5_hm071_contract()
  if (!identical(names(contract), names(frozen))) {
    g5_hm071_stop("Frozen HYP-MOM-07.1 contract field set changed.")
  }
  same <- vapply(names(frozen), function(field) identical(contract[[field]], frozen[[field]]), logical(1))
  if (!all(same)) {
    g5_hm071_stop(paste(
      "Frozen HYP-MOM-07.1 contract changed:",
...
g5_hm071_validate_bars <- function(bars, maximum_allowed_date, contract = g5_hm071_contract()) {
  contract <- g5_hm071_validate_contract(contract)
  required <- c("symbol", "session_date", "open", "close", "adjusted", "timeframe")
  missing <- setdiff(required, names(bars))
  if (length(missing)) {
    g5_hm071_stop(paste("Missing required bar columns:", paste(missing, collapse = ", ")))
  }
  ...
  strict_order <- all(vapply(per_symbol, function(z) all(diff(z$session_date) > 0), logical(1)))
  date_sets <- lapply(per_symbol, function(z) as.character(z$session_date))
  ...
  checks <- data.frame(
    ...
    passed = c(
      ...
      nrow(x) > 0L && maximum_observed < contract$qqew_mandate_change
    ),
    observed = c(
  ...
  if (!all(checks$passed)) {
    g5_hm071_stop(paste(
      "HYP-MOM-07.1 bar validation failed:",
      paste(checks$check_id[!checks$passed], collapse = ", ")
    ))
  }
  wide <- merge(
    per_symbol[[contract$cap_symbol]][c("session_date", "open", "close")],
    per_symbol[[contract$equal_symbol]][c("session_date", "open", "close")],
    by = "session_date", suffixes = c("_qqq", "_qqew"), all = FALSE, sort = TRUE
  )
  list(bars = x, wide = wide, checks = checks)
}

g5_hm071_zone_panel <- function(
  bars,
  zone_start,
  zone_end,
  ...
  checked <- g5_hm071_validate_bars(bars, maximum_allowed_date, contract)
  x <- checked$wide
  ...
  if (length(anchor_i) < contract$minimum_zone_anchors) {
    g5_hm071_stop(paste("Insufficient common anchors in zone:", length(anchor_i)))
  }
  ...
  x_e <- vapply(contract$lookback_grid, function(lag_n) {
    log(x$close_qqew[anchor_i] / x$close_qqew[anchor_i - lag_n])
  }, numeric(length(anchor_i)))
  y_q <- vapply(contract$target_grid, function(horizon_n) {
    log(x$open_qqq[anchor_i + 1L + horizon_n] / x$open_qqq[anchor_i + 1L])
  }, numeric(length(anchor_i)))
  y_e <- vapply(contract$target_grid, function(horizon_n) {
    log(x$open_qqew[anchor_i + 1L + horizon_n] / x$open_qqew[anchor_i + 1L])
  }, numeric(length(anchor_i)))
  colnames(x_q) <- colnames(x_e) <- paste0("L", contract$lookback_grid)
  colnames(y_q) <- colnames(y_e) <- paste0("H", contract$target_grid)
  matrices <- list(x_qqq = x_q, x_qqew = x_e, x_spread = x_q - x_e,
                   y_qqq = y_q, y_qqew = y_e, y_spread = y_q - y_e)
  if (!all(vapply(matrices, function(m) all(is.finite(m)), logical(1)))) {
    g5_hm071_stop("Nonfinite predictor or target values were constructed.")
  }
  list(
...
g5_hm071_cell_statistics <- function(x, y, lookback, target) {
...
g5_hm071_surface <- function(panel, contract = g5_hm071_contract()) {
  rows <- list()
  row_i <- 1L
  for (l_i in seq_along(contract$lookback_grid)) {
    for (h_i in seq_along(contract$target_grid)) {
      rows[[row_i]] <- g5_hm071_cell_statistics(
        panel$x_spread[, l_i], panel$y_spread[, h_i],
  ...
  if (nrow(out) != 9L || anyDuplicated(out$cell_id)) {
    g5_hm071_stop("Frozen nine-cell surface is incomplete or duplicated.")
  }
  out
}

g5_hm071_rotate_rows <- function(matrix, shift) {
  n <- nrow(matrix)
...
g5_hm071_shift_test <- function(panel, surface, contract = g5_hm071_contract()) {
  ...
  list(
    ...
    decision = data.frame(
      ...
      passed = passed,
      status = if (passed) "TRAIN_SEARCH_ADJUSTED_SURFACE_PASS" else "STOP_HYP_MOM_07_1_NO_SEARCH_ADJUSTED_TRAIN_SURFACE",
      stringsAsFactors = FALSE
    )
  )
}

g5_hm071_nominate <- function(surface, passed) {
...
g5_hm071_cell_vectors <- function(panel, lookback, target, contract = g5_hm071_contract()) {
  l_i <- match(as.integer(lookback), contract$lookback_grid)
  h_i <- match(as.integer(target), contract$target_grid)
  if (is.na(l_i) || is.na(h_i)) g5_hm071_stop("Requested cell is outside the frozen surface.")
  data.frame(
    anchor_date = panel$anchor_date,
    entry_date = panel$entry_date,
    maximum_exit_date = panel$maximum_exit_date,
    x_spread = panel$x_spread[, l_i],
    x_qqq = panel$x_qqq[, l_i],
    x_qqew = panel$x_qqew[, l_i],
    y_spread = panel$y_spread[, h_i],
    y_qqq = panel$y_qqq[, h_i],
    y_qqew = panel$y_qqew[, h_i],
    stringsAsFactors = FALSE
  )
}

g5_hm071_fit_coefficients <- function(y, predictors) {
  design <- cbind(1, as.matrix(predictors))
  fit <- stats::lm.fit(design, y)
  if (any(!is.finite(fit$coefficients))) g5_hm071_stop("Nonfinite frozen-model coefficient.")
  unname(fit$coefficients)
}

g5_hm071_model_predictions <- function(train, development) {
  definitions <- list(
    DRIFT = character(),
    SPREAD = "x_spread",
    QQQ_LEG = "x_qqq",
    QQEW_LEG = "x_qqew",
    TWO_LEG = c("x_qqq", "x_qqew")
  )
  rows <- lapply(names(definitions), function(model_id) {
    ...
    dev_x <- if (length(fields)) development[fields] else matrix(numeric(nrow(development) * 0L), nrow = nrow(development))
    coefficients <- g5_hm071_fit_coefficients(train$y_spread, train_x)
    design <- cbind(1, as.matrix(dev_x))
    prediction <- as.numeric(design %*% coefficients)
    error <- development$y_spread - prediction
    data.frame(
      model_id = model_id,
...
g5_hm071_stationary_indices <- function(n, expected_block) {
  p <- 1 / expected_block
  out <- integer(n)
...
g5_hm071_bootstrap_beta <- function(pairs, contract = g5_hm071_contract()) {
  set.seed(contract$bootstrap_seed)
  beta <- replicate(contract$bootstrap_count, {
    idx <- g5_hm071_stationary_indices(nrow(pairs), contract$bootstrap_expected_block)
    unname(stats::lm.fit(cbind(1, pairs$x_spread[idx]), pairs$y_spread[idx])$coefficients[[2L]])
  })
  stats::quantile(beta, probs = c(0.05, 0.95), type = contract$quantile_type, names = FALSE)
}

g5_hm071_years <- function(pairs) {
  years <- format(pairs$anchor_date, "%Y")
  do.call(rbind, lapply(split(seq_len(nrow(pairs)), years), function(idx) {
    stats <- g5_hm071_cell_statistics(pairs$x_spread[idx], pairs$y_spread[idx], NA_integer_, NA_integer_)
    data.frame(year = years[idx[[1L]]], anchor_count = length(idx), beta = stats$beta,
               correlation = stats$correlation, spearman = stats$spearman, stringsAsFactors = FALSE)
  }))
}

g5_hm071_phases <- function(pairs, horizon) {
  ...
  do.call(rbind, lapply(phases, function(phase) {
    ...
    stats <- g5_hm071_cell_statistics(pairs$x_spread[idx], pairs$y_spread[idx], NA_integer_, NA_integer_)
    data.frame(phase_offset = phase, anchor_count = length(idx), beta = stats$beta,
               correlation = stats$correlation, stringsAsFactors = FALSE)
  }))
}

g5_hm071_run_train <- function(bars, contract = g5_hm071_contract()) {
  panel <- g5_hm071_zone_panel(bars, contract$train_start, contract$train_end, contract$train_end, contract)
  surface <- g5_hm071_surface(panel, contract)
  shift <- g5_hm071_shift_test(panel, surface, contract)
  nominee <- g5_hm071_nominate(surface, shift$decision$passed[[1L]])
  list(
    contract = contract,
    panel = panel,
    surface = surface,
...
g5_hm071_run_development <- function(train_bars, development_bars, nominee, contract = g5_hm071_contract()) {
  if (!is.data.frame(nominee) || nrow(nominee) != 1L) g5_hm071_stop("Exactly one frozen TRAIN nominee is required.")
  train_panel <- g5_hm071_zone_panel(train_bars, contract$train_start, contract$train_end, contract$train_end, contract)
  development_panel <- g5_hm071_zone_panel(development_bars, contract$development_start, contract$development_end, contract$development_end, contract)
  lookback <- nominee$lookback_sessions[[1L]]
  target <- nominee$target_sessions[[1L]]
  train_pairs <- g5_hm071_cell_vectors(train_panel, lookback, target, contract)
  development_pairs <- g5_hm071_cell_vectors(development_panel, lookback, target, contract)
  development_stats <- g5_hm071_cell_statistics(development_pairs$x_spread, development_pairs$y_spread, lookback, target)
  interval <- g5_hm071_bootstrap_beta(development_pairs, contract)
  models <- g5_hm071_model_predictions(train_pairs, development_pairs)
  years <- g5_hm071_years(development_pairs)
  phases <- g5_hm071_phases(development_pairs, target)
  mse <- stats::setNames(models$development_mse, models$model_id)
  gates <- data.frame(
    gate_id = c("G1_INTEGRITY", "G2_DIRECTIONAL_TRANSPORT", "G3_RANK_TRANSPORT",
                "G4_DRIFT_VALUE_ADD", "G5_LEG_SPECIFICITY", "G6_TEMPORAL_STABILITY"),
    passed = c(
      all(development_panel$integrity$passed),
      development_stats$beta[[1L]] > 0 && interval[[1L]] > 0,
      development_stats$spearman[[1L]] > 0,
      mse[["SPREAD"]] < mse[["DRIFT"]],
      mse[["SPREAD"]] < mse[["QQQ_LEG"]] && mse[["SPREAD"]] < mse[["QQEW_LEG"]],
    ...
    observed = c(
      ...
      sprintf("beta=%.8f;lower90=%.8f", development_stats$beta[[1L]], interval[[1L]]),
      sprintf("spearman=%.8f", development_stats$spearman[[1L]]),
      sprintf("spread=%.12g;drift=%.12g", mse[["SPREAD"]], mse[["DRIFT"]]),
      sprintf("spread=%.12g;qqq=%.12g;qqew=%.12g", mse[["SPREAD"]], mse[["QQQ_LEG"]], mse[["QQEW_LEG"]]),
  ...
  overall <- if (all(gates$passed)) {
    "DEVELOPMENT_PASS_HYP_MOM_07_1_CONFIRMATION_REVIEW_REQUIRED"
  } else {
    "STOP_HYP_MOM_07_1_DEVELOPMENT_LEADERSHIP_PERSISTENCE_GATES_FAILED"
  }
  list(
    contract = contract,
    nominee = nominee,
    ...
    development_statistics = transform(development_stats, beta_lower_90 = interval[[1L]], beta_upper_90 = interval[[2L]]),
